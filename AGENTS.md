...
PR上のCodex等によるreview threadは、判断済みのまま未resolvedで放置しないでください。作業中に取得できる既存のreview threadがある場合、それを未処理のまま放置しないことをルールとします。ただし、新しいreviewが投稿されるまで無期限に待機することを完了条件にはしません。

既存のreview threadには以下のとおり対応してください。
