...
class WebhookSettingsValidatorTest {
  ...
  fun `URLがhttp・https以外のscheme場合はINVALID_URL`() {
    val result = WebhookSettingsValidator.validate("ftp://example.com", emptyList())
  ...
  fun `URLにhostがない場合はINVALID_URL`() {
    val result = WebhookSettingsValidator.validate("https:///path", emptyList())
  ...
  fun `URLとして構文解析できない場合はINVALID_URL`() {
    val result = WebhookSettingsValidator.validate("not a url", emptyList())
  ...
  fun `Header名が空白のみの場合はBLANK_HEADER_NAME`() {
    val result = WebhookSettingsValidator.validate(validUrl, listOf(WebhookHeader("  ", "v")))
  ...
  fun `Content-TypeヘッダーはRESERVED_CONTENT_TYPE_HEADER`() {
    val result = WebhookSettingsValidator.validate(validUrl, listOf(WebhookHeader("Content-Type", "text/plain")))
  ...
  fun `通常のHeader nameはvalidation errorを返さずtoken文字のみ許可される`() {
    val result = WebhookSettingsValidator.validate(validUrl, listOf(WebhookHeader("X-API-Key_9.test!~", "v")))
  ...
  fun `Header nameの前後空白はtrimされて保存用に正規化される`() {
    val result = WebhookSettingsValidator.validate(validUrl, listOf(WebhookHeader("  Authorization  ", "v")))
